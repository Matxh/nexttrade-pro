...
app.post('/api/stripe/checkout',async(req,res)=>{
  ...
  try{
    ...
    const baseUrl=process.env.BASE_URL||'https://priceaction.ai';
...
app.post('/api/stripe/portal',async(req,res)=>{
  ...
  try{const baseUrl=process.env.BASE_URL||'https://priceaction.ai';const s=await stripeRequest('billing_portal/sessions','POST',{customer:user.stripeCustomerId,return_url:baseUrl});res.json({url:s.url});}
  catch(err){res.status(500).json({error:err.message});}
});
...
app.post('/api/analyze',async(req,res)=>{
  ...
  try{
    console.log(`\n[PriceAction AI] ⚡ ${sym} ${tf} | Haiku→Sonnet→Opus`);
    const t0=Date.now();
    ...
    console.log(`[PriceAction AI] ⚡ Done in ${elapsed}s\n`);
    ...
    console.error('[PriceAction AI] Error:',err.message);
...
app.post('/api/manual-payment', async(req,res)=>{
  ...
  if(sgKey) {
    ...
    const body = `New manual payment received!\n\nEmail: ${email}\nPlan: ${plan} (${prices[plan]||'?'})\nMethod: ${type}\nReference: ${reference}\nTime: ${new Date().toISOString()}\n\nTo activate this account, call this URL:\n${process.env.BASE_URL||'https://priceaction.ai'}/api/activate-manual?email=${encodeURIComponent(email)}&plan=${plan}&secret=${process.env.ADMIN_SECRET||'changeme'}\n\nOr reply to ${email} to ask for more info.`;
    try {
      await fetch('https://api.sendgrid.com/v3/mail/send',{method:'POST',headers:{'Authorization':`Bearer ${sgKey}`,'Content-Type':'application/json'},body:JSON.stringify({personalizations:[{to:[{email:process.env.ADMIN_EMAIL||process.env.FROM_EMAIL||'admin@nexttrade-ai.com'}]}],from:{email:process.env.FROM_EMAIL||'noreply@nexttrade-ai.com',name:'PriceAction AI'},subject:`Manual Payment: ${email} wants ${plan}`,content:[{type:'text/plain',value:body}]})});
